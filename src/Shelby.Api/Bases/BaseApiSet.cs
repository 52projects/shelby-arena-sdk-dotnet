...
namespace Shelby.Api {
    public abstract class BaseApiSet<T> where T : new() {
        #region Properties
        private readonly RequestCredentials _requestCredentials;
        ...
        public virtual bool Create(byte[] stream, string url = "") {
            ...
            if (!string.IsNullOrWhiteSpace(url)) {
                ...
                targetUrl = url.Substring(this._requestCredentials.BaseUrl.Length);
            }
        ...
        public virtual bool Create<S>(S entity, string url = "") where S : new() {
            ...
            if (!string.IsNullOrWhiteSpace(url)) {
                if (url.Trim().StartsWith(this._requestCredentials.BaseUrl)) {
                    if (url.Trim().Length <= this._requestCredentials.BaseUrl.Length) {
                        throw new Exception("Invalid url: " + url);
                    }
                    targetUrl = url.Substring(this._requestCredentials.BaseUrl.Length);
        ...
        internal IRestResponse<T> ExecuteRequest(IRestRequest request) {
            ...
            if ((int)response.StatusCode >= 300) {
                ...
                if (location != null) {
                    request = CreateRestRequest(Method.GET, location.Value.ToString().Substring(this._requestCredentials.BaseUrl.Length));
